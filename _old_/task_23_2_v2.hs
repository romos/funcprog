...
show' (Resistance n) (beginRow, width, height) strings =
    let
      ...
      leftCabelLen = (width - exactWidth) `div` 2
      leftCabel = generateString "-" leftCabelLen
      rightCabel = generateString "-" (width - exactWidth - leftCabelLen)
      spaces = generateString " " width
    in [if k == beginRow
        then curString ++ leftCabel ++ "-" ++ show n ++ "-" ++ rightCabel
